...
#' @param plot_title_position Position of title/subtitle relative to panel (`"panel"`) or full plot (`"plot"`).
#' @param plot_caption_position Position of caption relative to panel (`"panel"`) or full plot (`"plot"`).
...
theme_grid <- function(
  ...
  if (base_family != "" && title_family == "") {
    title_family <- base_family
  }
  if (base_family != "" && subtitle_family == "") {
    subtitle_family <- base_family
  }
  if (base_family != "" && strip_text_family == "") {
    strip_text_family <- base_family
  }
  if (base_family != "" && caption_family == "") {
    caption_family <- base_family
  }
  if (base_family != "" && axis_text_family == "") {
    axis_text_family <- base_family
  }
  if (base_family != "" && axis_title_family == "") {
  ...
  if (inherits(grid, "character") || grid == TRUE) {
    ret <- ret +
      theme(panel.grid = element_line(color = grid_color, linewidth = 0.2))
    ret <- ret +
      theme(
