...
Finally load fonts for use. Run once per session.
...
Comparison of default theme, inter, plex and nunito.

To permanently set theme for a session, use
...
Custom color scale, legend in the top and only horizontal gridlines.
...
Plain theme with no axis and no gridlines.
...
To permanently set palette for a session, use
...
It is also possible to use custom fonts by providing font names to the `family` arguments in `theme_custom()`.